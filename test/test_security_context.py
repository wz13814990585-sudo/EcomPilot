...
def test_trusted_security_identity_wins_and_reserved_params_are_removed():
    ...
    assert (ctx.tenant_id, ctx.user_id, ctx.store_id) == ("tenant-1", "user-1", "store-1")
...
def test_legacy_task_context_identity_remains_compatible_but_untrusted():
    ctx = normalize_task_context({"query": "q", "tenant_id": "legacy-t", "user_id": "legacy-u"})
...
def test_production_agent_ingress_requires_trusted_security():
