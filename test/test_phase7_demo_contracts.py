...
def test_init_db_keeps_postgres_do_blocks_intact():
    ...
    for name in ("business_tables.sql", "vector_tables.sql"):
        ...
        do_blocks = [
            statement for statement in statements if statement.lstrip().startswith("DO $$")
        ]
...
def test_ci_and_docker_artifacts_keep_phase7_delivery_contracts():
    ...
    assert 'pip install -e ".[dev]"' in ci and "pytest -q" in ci
    assert "ruff check ecom_agent_matrix test" in ci
    assert "ruff format --check ecom_agent_matrix test" in ci
