...
def test_shutdown_cancels_agent_and_closes_all_owned_clients():
    async def scenario():
        ...
        with (
            patch.object(main_module, "cancel_master_tasks", new=AsyncMock()) as master,
            patch.object(main_module, "close_http_session", new=AsyncMock()) as llm,
            patch.object(main_module.AsyncRedisClient, "close", new=AsyncMock()) as redis,
            patch.object(main_module.AsyncPGClient, "close", new=AsyncMock()) as postgres,
        ):
...
def test_readiness_reports_dependencies_and_llm_degraded_is_optional():
    async def scenario():
        with (
            patch(
                "ecom_agent_matrix.api.health.check_postgres",
                new=AsyncMock(return_value={"ok": True, "status": "ok"}),
            ),
            patch(
                "ecom_agent_matrix.api.health.check_redis",
                new=AsyncMock(return_value={"ok": True, "status": "ok"}),
            ),
            patch("ecom_agent_matrix.core.llm.router.is_llm_configured", return_value=False),
            patch("ecom_agent_matrix.api.health.settings.LLM_REQUIRED_FOR_READINESS", False),
        ):
        ...
        with (
            ...
            patch("ecom_agent_matrix.core.llm.router.is_llm_configured", return_value=False),
            patch("ecom_agent_matrix.api.health.settings.LLM_REQUIRED_FOR_READINESS", True),
        ):
