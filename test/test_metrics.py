...
def test_http_and_agent_metrics_increment_without_high_cardinality_labels():
    before_http = _value(
        "http_requests_total", {"method": "POST", "route": "/api/v1/tasks", "status_class": "2xx"}
    )
    ...
    assert (
        _value(
            "http_requests_total",
            {"method": "POST", "route": "/api/v1/tasks", "status_class": "2xx"},
        )
        == before_http + 1
    )
    assert (
        _value("agent_tasks_total", {"agent": "data_query", "status": "success"})
        == before_agent + 1
    )
    assert (
        _value("agent_tasks_total", {"agent": "data_query", "status": "failure"})
        == before_failure + 1
    )
    assert (
        _value("http_request_duration_seconds_count", {"method": "POST", "route": "/api/v1/tasks"})
        >= 1
    )


def test_skill_executor_emits_timeout_and_normal_skill_metrics():
    before_timeout = _value(
        "skill_executions_total",
        {"skill": "test_metrics_timeout", "status": "failure", "error_code": "SKILL_TIMEOUT"},
    )
...
def test_approval_required_metric_label_is_bounded():
    before = _value(
        "skill_executions_total",
        {"skill": "record_order_risk", "status": "failure", "error_code": "APPROVAL_REQUIRED"},
    )
...
def test_llm_metrics_tokens_cost_and_rule_fallback_behavior():
    ...
    before = _value(
        "llm_calls_total", {"provider": "openai", "purpose": "planner", "status": "success"}
    )
    metrics.observe_llm("openai", "planner", True, 0.2, 7, 3, 0.001)
    assert (
        _value("llm_calls_total", {"provider": "openai", "purpose": "planner", "status": "success"})
        == before + 1
    )
    ...
    async def fallback():
        with patch(
            "ecom_agent_matrix.modules.utils.llm_explain.is_llm_configured", return_value=False
        ):
    ...
    calls_before = sum(
        ...
        if sample.name == "llm_calls_total"
    )
...
def test_real_provider_invocation_emits_usage_and_estimated_cost_metrics():
    ...
    async def scenario():
        with (
            patch.object(settings, "OPENAI_API_KEY", "configured"),
            patch.object(settings, "OPENAI_CHAT_MODEL", "gpt-fixed"),
            patch.object(
                settings,
                "LLM_PRICE_TABLE",
                {"openai:gpt-fixed": {"input_per_1m": 1, "output_per_1m": 2}},
            ),
            patch(
                "ecom_agent_matrix.core.llm.provider.get_http_session",
