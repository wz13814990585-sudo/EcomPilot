...
from ecom_agent_matrix.agents.query.agent import _ensure_sku, run_query
        ...
        with patch(
            ...
            new=AsyncMock(
                return_value=WorkflowResult(
                    success=True,
                    data={"best_sku": "SKU-1", "candidates": [{"sku": "SKU-1"}]},
                )
            ),
        ):
...
def test_run_exec_dict_remains_compatible_after_handoff():
    async def scenario():
        with patch(
            "ecom_agent_matrix.agents.exec.agent.run_report_workflow",
            new=AsyncMock(return_value=WorkflowResult(success=True)),
        ):
