...
def _reply(*, llm_ok=True, success=True, answer="answer"):
    return SkillResult(
        success=success,
        error_code="SKILL_EXECUTION_ERROR" if not success else "",
        error_msg="failed" if not success else "",
        data={
            "answer": answer,
            "llm_ok": llm_ok,
            "rag_used": False,
...
def test_crm_payload_fake_task_id_cannot_override_envelope():
    request = parse_crm_request(
        normalize_task_context({"query": "hello", "task_id": "FAKE"}, task_id="REAL")
    )
...
def test_crm_no_longer_calls_text_translate_and_passes_language_to_reply():
    ...
    async def scenario():
        with (
            patch(
                "ecom_agent_matrix.workflows.crm.workflow.AgentShortMemory", return_value=_memory()
            ),
            patch("ecom_agent_matrix.workflows.crm.workflow.exec_skill", side_effect=skills),
        ):
            return await run_crm_workflow(
                {"query": "refund please", "lang": "en", "session_id": "S"}
            )
...
def test_crm_short_memory_read_failure_is_best_effort():
    ...
    async def scenario():
        with (
            patch("ecom_agent_matrix.workflows.crm.workflow.AgentShortMemory", return_value=memory),
            patch(
...
def test_crm_taobao_failure_and_reply_success_is_partial():
    ...
    async def scenario():
        with (
            ...
            return await run_crm_workflow(
                {"query": "check ORD-1", "use_taobao": True, "order_no": "ORD-1"}
            )

    result = asyncio.run(scenario())
    assert result.success and result.partial_success
    assert result.error_code == PARTIAL_SUCCESS
    assert result.metadata["skill_error_codes"]["taobao_api"] == "SKILL_TIMEOUT"
...
def test_crm_reply_fallback_is_structured_degradation():
    async def scenario():
        with (
            ...
            patch(
                "ecom_agent_matrix.workflows.crm.workflow.exec_skill",
                new=AsyncMock(return_value=_reply(llm_ok=False)),
            ),
        ):
...
def test_crm_total_reply_failure_without_fallback_fails():
    async def scenario():
        with (
            ...
            patch(
                "ecom_agent_matrix.workflows.crm.workflow.exec_skill",
                new=AsyncMock(return_value=_reply(success=False, answer="")),
            ),
            patch("ecom_agent_matrix.workflows.crm.workflow._safe_fallback", return_value=""),
        ):
            return await run_crm_workflow({"query": "hello"})
