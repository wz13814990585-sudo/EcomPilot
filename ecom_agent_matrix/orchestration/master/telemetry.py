"""Master provider call budget and token usage."""
...
class MasterLLMTelemetry:
    def __init__(self, max_calls: int | None = None):
        self.max_calls = int(max_calls if max_calls is not None else settings.MASTER_MAX_LLM_CALLS)
        self._usage = {
            "planner": LLMUsage(),
            "recovery": LLMUsage(),
            "polish": LLMUsage(),
    ...
    def add_result(self, kind: UsageKind, result: ChatResult) -> None:
        ...
        self._usage[kind] = current.model_copy(
            update={
                ...
                "completion_tokens": current.completion_tokens + int(result.completion_tokens or 0),
