...
_RULES: tuple[tuple[str, str, re.Pattern[str]], ...] = (
    ...
    (
        ...
        "RULE_AD_OPTIMIZE",
        re.compile(
            r"优化.*广告|广告.*优化|调整出价|调出价|投放优化|optimi[sz]e.*(?:ad|campaign)", re.I
        ),
    ),
    (
        "ops_report",
    ...
    (
        ...
        "RULE_CRM",
        re.compile(
            r"回复.*(?:客户|顾客|买家)|(?:客服|售后).*(?:回复|处理)|帮我回复|customer\s+(?:reply|support)",
            re.I,
        ),
    ),
    (
    ...
    (
        ...
        "RULE_GOODS_CATALOG",
        re.compile(
            r"商品总数|商品数量|列出.*商品|全部商品|商品目录|product\s+catalog|list\s+all\s+products",
            re.I,
        ),
    ),
    (
