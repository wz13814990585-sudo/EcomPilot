"""Master routing policy and DAG validation."""
...
def validate_master_plan(plan: MasterPlan) -> MasterPlan:
    ...
    for step in plan.steps:
        ...
        if step.task_type not in ALLOWED_TASK_TYPES:
            raise MasterPlanValidationError(INVALID_TASK_TYPE, f"未知 task_type: {step.task_type}")
        ...
        if missing:
            raise MasterPlanValidationError(INVALID_DEPENDENCY, f"依赖不存在: {', '.join(missing)}")
