...
def _request_from_row(row) -> ApprovalRequest:
    return ApprovalRequest(
        ...
        tenant_id=row[1],
        store_id=row[2],
        requester_user_id=row[3],
        approver_user_id=row[4] or "",
        task_id=row[5],
        skill_name=row[6],
        params_hash=row[7],
        status=row[8],
...
class ApprovalService:
    ...
    async def get_request(
        self, approval_id: str, security: SecurityContext
        ...
        rows = await AsyncPGClient.execute_write(
            ...
            [approval_id, scope.tenant_id, scope.store_id],
            scope=scope,
        )
    ...
    async def approve(self, approval_id: str, security: SecurityContext) -> ApprovalGrant:
        ...
        reason = (
            "ADMIN_SELF_APPROVAL"
            if is_admin and request.requester_user_id == security.user_id
            else ""
        )
        rows = await AsyncPGClient.execute_write(
            """
            ...
            """,
            [security.user_id, now, reason, approval_id, scope.tenant_id, scope.store_id, now],
            scope=scope,
        )
        if not rows:
        ...
        return ApprovalGrant(
            ...
            approver_user_id=security.user_id,
            skill_name=request.skill_name,
    ...
    async def consume(
        self, grant: ApprovalGrant, *, context, skill_name: str, params_hash: str
    ) -> None:
        now = datetime.now(timezone.utc)
        ...
        if (
            ...
            or grant.store_id != context.store_id
            or grant.skill_name != skill_name
        ...
        rows = await AsyncPGClient.execute_write(
            ...
            RETURNING approval_id
            """,
            [now, grant.approval_id, scope.tenant_id, scope.store_id, skill_name, params_hash, now],
            scope=scope,
        )
        if not rows:
